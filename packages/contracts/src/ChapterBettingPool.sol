...
/// @title ChapterBettingPool
/// @notice Parimutuel betting pool for story chapter branches
/// @dev Pool settles with 85% to winners, 12.5% treasury, 2.5% ops
contract ChapterBettingPool is Ownable, ReentrancyGuard {
    ...
    event BetPlaced(address indexed bettor, uint8 branchIndex, uint256 amount, bool isAgent);
    ...
    function lockPool() external onlyOwner {
        ...
        emit PoolLocked(totalPoolAmount);
    }
    ...
    function settlePool(uint8 _winningBranch) external onlyOwner {
        ...
        emit PoolSettled(_winningBranch, totalPoolAmount - treasuryAmount - opAmount, treasuryAmount, opAmount);
    }

    /// @notice Claim rewards for all winning bets
    /// @dev Calculates pro-rata share of winner pool
    ...
    /// @notice Cancel pool and refund all bets
    /// @dev Emergency function, can only be called before settlement
    ...
    function getPendingReward(address _user) external view returns (uint256) {
        ...
        for (uint256 i = 0; i < indices.length; i++) {
            Bet storage bet = bets[indices[i]];
            if (bet.branchIndex == winningBranch && !bet.claimed) {
                total += (bet.amount * winnerPool) / branches[winningBranch].totalBets;
            }
        }
        return total;
    }
    ...
    function getBranchInfo(uint8 _branchIndex) external view returns (string memory hash, uint256 total, uint256 count) {
        require(_branchIndex < branchCount, "Invalid branch");
